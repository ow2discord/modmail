...
module.exports = ({ bot, knex, config, commands }) => {
	...
	async function applyScheduledSuspensions() {
		const threadsToBeSuspended =
			await threads.getThreadsThatShouldBeSuspended();
	...
	commands.addInboxThreadCommand(
		...
		async (msg, args, thread) => {
			...
			if (args.delay) {
				...
				await thread.scheduleSuspend(
					suspendAt.format("YYYY-MM-DD HH:mm:ss"),
					msg.author,
				);

				thread.postSystemMessage(
					`Thread will be suspended in ${utils.humanizeDelay(args.delay)}. Use \`${config.prefix}suspend cancel\` to cancel.`,
				);
	...
	commands.addInboxServerCommand(
		...
		async (msg, _args, thread) => {
			...
			const otherOpenThread = await threads.findOpenThreadByUserId(
				thread.user_id,
			);
			if (otherOpenThread) {
				thread.postSystemMessage(
