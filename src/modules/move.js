		...
		async (msg, args, thread) => {
			...
			if (config.syncPermissionsOnMove) {
				const newPerms = Array.from(
					targetCategory.permissionOverwrites.map((ow) => {
						return {
							id: ow.id,
							type: ow.type,
							allow: ow.allow,
				...
				try {
					await bot.requestHandler.request(
						"PATCH",
						Routes.channel(thread.channel_id),
						true,
						{
					...
					thread.postSystemMessage(
						`Thread moved to ${targetCategory.name.toUpperCase()}, but failed to sync permissions: ${e.message}`,
					);
					return;
				}
			}
