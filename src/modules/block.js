...
export default ({ bot, config, commands }) => {
	...
	async function removeExpiredBlocks() {
		const expiredBlocks = blocked.getExpiredBlocks();
	...
	const blockCmd = async (msg, args, thread) => {
		const userIdToBlock = args.userId || thread?.user_id;
		...
		const expiresAt = args.blockTime
			? utc().add(args.blockTime, "ms").format("YYYY-MM-DD HH:mm:ss")
			: null;
		...
		if (expiresAt) {
			...
			if (timedBlockMessage) {
				...
				dmChannel.createMessage(formatted).catch(noop);
			}
		} else {
			msg.channel.createMessage(
			...
			if (config.blockMessage != null) {
				const dmChannel = await user.getDMChannel();
				dmChannel.createMessage(config.blockMessage).catch(noop);
			}
		}
	};
	...
	const unblockCmd = async (msg, args, thread) => {
		const userIdToUnblock = args.userId || thread?.user_id;
		...
		const unblockAt = args.unblockDelay
			? utc().add(args.unblockDelay, "ms").format("YYYY-MM-DD HH:mm:ss")
			: null;
		...
		if (unblockAt) {
			...
			if (config.unblockMessage) {
				const dmChannel = await user.getDMChannel();
				dmChannel.createMessage(config.unblockMessage).catch(noop);
			}
		}
	};
