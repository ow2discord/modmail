...
export interface ModmailConfig {
	...
	timedBlockMessage?: string | null;
	unblockMessage?: string | null;
	timedUnblockMessage?: string | null;
	...
	overrideRoleNameDisplay?: string | null;
...
if (requestedConfigFile) {
	try {
		...
		if (e.code === "ENOENT") {
			...
			console.error(
				`Error reading specified config file ${requestedConfigFile}: ${e.message}`,
			);
		}
...
if (configFileToLoad) {
	...
	try {
		const raw = fs.readFileSync(srcRelativePath, { encoding: "utf8" });
...
// FIXME: Valid this can be killed off
//
