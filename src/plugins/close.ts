	...
	commands.addGlobalCommand(
		...
		async (msg, args) => {
			...
			if (msg.channel instanceof DMChannel) {
				...
				// Timed close
				try {
					const delay = getDelayFromArgs(opts);
					await thread.scheduleClose(delay, msg.author, silentClose);

					if (silentClose)
						thread.postSystemMessage(
							`Thread is now scheduled to be closed silently in ${humanizeDelay(delay)}. Use \`${config.prefix}close cancel\` to cancel.`,
						);
					else
						thread.postSystemMessage(
							`Thread is now scheduled to be closed in ${humanizeDelay(delay)}. Use \`${config.prefix}close cancel\` to cancel.`,
						);
				} catch (e: unknown) {
					thread.postSystemMessage(`${e}`);
				}
