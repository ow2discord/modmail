exports.up = async (knex) => {
	await knex.schema.renameTable(
		"moderator_role_overrides",
		"old_moderator_role_overrides",
	);
	...
	const rows = await knex.table("old_moderator_role_overrides").select();
...
exports.down = async (knex) => {
	...
	const rows = await knex.table("new_moderator_role_overrides").select();

	if (rows.length) {
		await knex.table("moderator_role_overrides").insert(
			rows.map((r) => {
				delete r.id;
				return r;
			}),
