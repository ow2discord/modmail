...
async function setModeratorDefaultRoleOverride(moderatorId, roleId) {
	const existingGlobalOverride =
		await getModeratorDefaultRoleOverride(moderatorId);
	if (existingGlobalOverride) {
		...
		await knex("moderator_role_overrides").insert({
...
async function setModeratorThreadRoleOverride(moderatorId, threadId, roleId) {
	const existingGlobalOverride = await getModeratorThreadRoleOverride(
		moderatorId,
		threadId,
	);
	if (existingGlobalOverride) {
